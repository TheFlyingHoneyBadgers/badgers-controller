...
echo "=========================="
echo "Installing App badgers"

# Here is where we perform installation of scripts, services, etc.
echo " - Installing ROS packages for badgers..."
...
runuser -l $LIBPANDA_USER -c /etc/libpanda.d/apps/badgers/installRosPackages.sh
...
rosrun robot_upstart install badgerscontroller/launch/${LAUNCH_FILE} --user root

echo "Enabling can_to_ros startup script"
sudo systemctl daemon-reload
sudo systemctl enable badgerscontroller
popd
echo "@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@"
