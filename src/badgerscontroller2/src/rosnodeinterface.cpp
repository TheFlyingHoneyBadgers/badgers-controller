...
// C/C++ source code generated on : Tue Nov 28 19:05:26 2023
//
  ...
  namespace matlab
  {
    ...
    void NodeInterface::initialize(int argc, char ** argv)
    {
      try {
        ...
        ROS_INFO("** Starting the model \"badgerscontroller2\" **\n");

        // initialize the model which will initialize the publishers and subscribers
        rtmSetErrorStatus(badgerscontroller2_M, (NULL));
        badgerscontroller2_initialize();
    ...
    boolean_T NodeInterface::getStopRequestedFlag(void)
    {
      ...
      return (!(rtmGetErrorStatus(badgerscontroller2_M)
                == (NULL)));

#else

      return (!(rtmGetErrorStatus(badgerscontroller2_M)
                == (NULL)) || rtmGetStopRequested(badgerscontroller2_M));
    ...
    {
      mRunModel = (rtmGetErrorStatus(badgerscontroller2_M) ==
                   (NULL));
      while (mRunModel) {
        mBaseRateSem.wait();
