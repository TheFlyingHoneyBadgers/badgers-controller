...
// C/C++ source code generated on : Mon Nov 27 22:28:26 2023
//
  ...
  namespace matlab
  {
    ...
    void NodeInterface::initialize(int argc, char ** argv)
    {
      try {
        ...
        ROS_INFO("** Starting the model \"badgerscontroller\" **\n");

        // initialize the model which will initialize the publishers and subscribers
        rtmSetErrorStatus(badgerscontroller_M, (NULL));
        badgerscontroller_initialize();
    ...
    boolean_T NodeInterface::getStopRequestedFlag(void)
    {
      ...
      return (!(rtmGetErrorStatus(badgerscontroller_M)
                == (NULL)));

#else

      return (!(rtmGetErrorStatus(badgerscontroller_M)
                == (NULL)) || rtmGetStopRequested(badgerscontroller_M));
    ...
    {
      mRunModel = (rtmGetErrorStatus(badgerscontroller_M) ==
                   (NULL));
      while (mRunModel) {
        mBaseRateSem.wait();
